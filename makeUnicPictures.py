...
def get_unic_from_dict(d, prefix):
    ...
    print(f'{prefix}: in {len(d)}; out {len(unic_combinations)}')
    return unic_combinations


for c, i in enumerate(good_images, 1):
    current_prefix = i.replace('flip', '')[:10]
    if current_prefix == prefix:
        temp_combinations.update({i: old_config[i]})
    else:
        all_combinations.update(get_unic_from_dict(temp_combinations, prefix))
